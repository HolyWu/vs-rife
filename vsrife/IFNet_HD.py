...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale=1.0):
        ...
        flow0 = self.block0(x)
        F1 = flow0
        warped_img0 = warp(x[:, :3], F1, self.device)
        warped_img1 = warp(x[:, 3:], -F1, self.device)
        flow1 = self.block1(torch.cat((warped_img0, warped_img1, F1), 1))
        F2 = (flow0 + flow1)
        warped_img0 = warp(x[:, :3], F2, self.device)
        warped_img1 = warp(x[:, 3:], -F2, self.device)
        flow2 = self.block2(torch.cat((warped_img0, warped_img1, F2), 1))
        F3 = (flow0 + flow1 + flow2)
        warped_img0 = warp(x[:, :3], F3, self.device)
        warped_img1 = warp(x[:, 3:], -F3, self.device)
