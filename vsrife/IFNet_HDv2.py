...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale=1.0):
        ...
        warped_img0 = warp(x[:, :3], F3_large[:, :2], self.device)
        warped_img1 = warp(x[:, 3:], F3_large[:, 2:4], self.device)
        flow3 = self.block3(torch.cat((warped_img0, warped_img1, F3_large), 1))
        F4 = (flow0 + flow1 + flow2 + flow3)
        if scale != 1.0:
            F4 = F.interpolate(F4, scale_factor=1 / scale, mode="bilinear", align_corners=False, recompute_scale_factor=False) / scale
